...
def export_stats_pdf():
    ...
    # Title
    title_style = ParagraphStyle('TitleStyle', parent=styles['Title'], fontName='Helvetica-Bold', fontSize=18, spaceAfter=20)
    subtitle_style = ParagraphStyle('SubtitleStyle', parent=styles['Normal'], fontName='Helvetica', fontSize=12, spaceAfter=20, alignment=1)
    
    niveau_text = niveau if niveau and niveau != "all" else "Toutes les classes"
    elements.append(Paragraph(f"Rapport Statistique - Trimestre {trim}", title_style))
    elements.append(Paragraph(f"{school_name} | {nom_prof} | {subject_name} | {niveau_text} | {selected_school_year}", subtitle_style))
    ...
    for r in class_rows:
        class_data.append([str(r["niveau"]), str(r["total"]), str(round(float(r["avg_moy"] or 0), 2))])
    if len(class_data) > 1:
    ...
    for i, r in enumerate(top_rows, 1):
        top_data.append([str(i), r["nom_complet"], r["niveau"], str(float(r["moyenne"]))])
    if len(top_data) > 1:
    ...
    for r in risk_rows:
        risk_data.append([r["nom_complet"], r["niveau"], str(float(r["moyenne"]))])
    if len(risk_data) > 1:
    ...
    filename = f"statistiques_T{trim}_{subject_name}_{niveau_text}.pdf"
...
print("Appended export_stats_pdf successfully")
